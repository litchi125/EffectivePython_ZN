...
* 在生成器中提供异常行为的更好方法是使用实现--iter--方法的类以及导致异常状态转换的方法，例如上面的Reset方法
...
​		使用tee将一个迭代器分割成由第二个形参指定的并行迭代器数量。如果迭代器的进度不一样，则该函数的内存使用量会增加，因为需要缓存来将挂起的项排队:
...
​		takewhile从迭代器中返回项，直到谓词函数为项返回false
...
​		dropwhile与takewhile相反，它跳过迭代器中的项，直到谓词函数第一次返回True:
...
​		filter False与filter内置函数相反，返回迭代器中的所有项，而predicate函数返回False:
